---
name: kb-write-docs
description: Inspect source material, split it into retrieval-focused documents, and write confirmed content into this project's configured knowledge-base directory (`wiki`). Use when Codex needs to import, copy, move, rename, merge, split, overwrite, or reorganize local knowledge-base documentation while protecting existing files and asking before indexing.
---
...
- Only modify files under the configured content directory (`wiki`). Never write outside it.
