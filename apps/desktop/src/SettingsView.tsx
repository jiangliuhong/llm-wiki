...
export default function SettingsView({ workspace, kbStats, knownWorkspaceCount, onOpenWorkspaceMenu, onIndexed, onWorkspaceRenamed }: SettingsViewProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    void invoke<PiEnvironmentInfo>("pi_environment_check")
      .then((info) => { if (!cancelled) setPiInfo(info); })
      .catch((reason: unknown) => { if (!cancelled) setPiError(String(reason)); });
    return () => { cancelled = true; };
  }, []);
  ...
  const saveExcludeRules = async (next: string[]): Promise<void> => {
    ...
    setExcludeError(null);
    try {
      const config = await invoke<KbConfigInfo>("kb_config_set_exclude", { root: workspace.root, exclude: next });
      setExcludeRules(config.exclude);
    } catch (reason: unknown) {
      ...
      <div className="sg-scroll">
        ...
        <nav className="sg-nav-tabs" role="tablist">
          ...
          >
            ...
            <span>应用全局设置</span>
            {piInfo?.hasUpdate && <span className="sg-badge warning" style={{ fontSize: 10 }}>有更新</span>}
          </button>
        </nav>
        ...
        {activeTab === "workspace" && (
          <>
            <p className="sg-tab-lead">
              当前工作区专属配置，规则与数据库独立保存在 <code>{workspace ? `${workspace.title}/.llm-wiki/` : ".llm-wiki/"}</code> 中。
            </p>
            ...
            {!workspace ? (
              <div className="sg-empty-state">
                ...
                <button type="button" className="sg-button primary" onClick={onOpenWorkspaceMenu}>选择工作区</button>
              </div>
            ) : (
              <>
                      ...
                      {editingTitle ? (
                        <div className="sg-title-edit">
                          <input
                            ...
                            autoFocus
                          />
                          <button type="button" className="sg-inline-btn primary" disabled={titleSaving || !titleInput.trim()} onClick={() => void handleSaveTitle()}>
                            {titleSaving ? "保存中…" : "保存"}
                          </button>
                          <button type="button" className="sg-inline-btn" disabled={titleSaving} onClick={() => { setEditingTitle(false); setTitleInput(workspace?.title ?? ""); }}>
                            取消
                          </button>
                        </div>
                      ) : (
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          ...
                          {inTauriRuntime() && (
                            <button type="button" className="sg-inline-btn" onClick={() => { setEditingTitle(true); setTitleInput(workspace.title); setTitleError(null); }}>
                              修改名称
                            </button>
                          )}
                        </div>
                ...
                <section className="sg-section">
                  <div className="sg-section-head">
                    ...
                    <div>
                      <strong>索引目录</strong>
                      <p>相对于工作区根目录的扫描路径，修改后需重新索引生效。默认为 <code>{defaultDirs.join(" / ") || "wiki / docs"}</code>。</p>
                    </div>
                    {includeDirs.length !== defaultDirs.length || !defaultDirs.every((d) => includeDirs.includes(d)) ? (
                      <button type="button" className="sg-button" disabled={dirSaving} onClick={() => void saveIncludeDirs(defaultDirs)}>恢复默认</button>
                    ) : null}
                  </div>
                  ...
                  >
                    ...
                    />
                    <button type="submit" className="sg-button" disabled={dirSaving || !dirInput.trim()}>添加目录</button>
                  </form>
                  {dirError && <p className="sg-error">错误:{dirError}</p>}
                  <p className="sg-hint">至少保留一个目录；路径必须位于工作区内，不支持 <code>..</code> 或绝对路径。排除项将在下方规则中生效。</p>
                </section>

                {/* 1.3 Ignore / Exclude Rules */}
                <section className="sg-section">
                  <div className="sg-section-head">
                    <span className="sg-section-icon"><Icon name="ban" /></span>
                    <div>
                      <strong>忽略文档与目录</strong>
                      <p>匹配的文件或目录将不会被扫描和索引，例如 <code>AGENTS.md</code> 或 <code>*.log</code>。修改后重新索引生效。</p>
                    </div>
                    {excludeRules.length !== defaultExclude.length || !defaultExclude.every((d) => excludeRules.includes(d)) ? (
                      <button type="button" className="sg-button" disabled={excludeSaving} onClick={() => void saveExcludeRules(defaultExclude)}>恢复默认</button>
                    ) : null}
                  </div>
                  <div className="sg-dirs">
                    {excludeRules.map((rule) => (
                      <span key={rule} className="sg-dir-chip">
                        ...
                        <button
                          ...
                          onClick={() => void saveExcludeRules(excludeRules.filter((v) => v !== rule))}
                        >
                          ×
                        </button>
                      </span>
                  ...
                  <form
                    className="sg-dir-form"
                    onSubmit={(event) => { event.preventDefault(); addExcludeRule(); }}
                  >
                    <input
                      value={excludeInput}
                      placeholder="例如 AGENTS.md 或 *.tmp 或 docs/drafts"
                      disabled={excludeSaving}
                      onChange={(event) => { setExcludeInput(event.target.value); setExcludeError(null); }}
                    />
                    <button type="submit" className="sg-button" disabled={excludeSaving || !excludeInput.trim()}>添加忽略</button>
                  </form>
                  {excludeError && <p className="sg-error">错误:{excludeError}</p>}
                  <p className="sg-hint">支持精确文件名（如 <code>AGENTS.md</code>）、通配符扩展名（如 <code>*.tmp</code>）或子目录名。修改后点击重新索引即刻生效。</p>
                </section>
        ...
        {activeTab === "app" && (
          <>
            ...
            <section className="sg-section">
              ...
              <div className="sg-kv">
                <div className="sg-kv-row">
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    <strong>{piInfo?.message ?? (piChecking ? "正在检测 Pi 运行环境…" : "点击右上角「Pi 检查」检测运行环境")}</strong>
                    {piInfo?.hasUpdate && <span className="sg-badge warning">发现新版本</span>}
                    {piInfo?.status === "ready" && !piInfo.hasUpdate && <span className="sg-badge success">已是最新</span>}
                  </div>
                </div>
                <div className="sg-kv-row">
                  <span>Node.js / Pi CLI</span>
                  <strong>{piInfo?.nodeVersion ? `Node ${piInfo.nodeVersion}` : "未检测到 Node"} · {piInfo?.piVersion ? `Pi CLI ${piInfo.piVersion}` : "未安装全局 Pi CLI"}</strong>
                </div>
                <div className="sg-kv-row">
                  <span>官方最新版本</span>
                  <strong>{piInfo?.latestVersion ? `npm @earendil-works/pi-coding-agent @ ${piInfo.latestVersion}` : "—"}</strong>
                </div>
                <div className="sg-kv-row">
                  <span>运行协议</span>
              ...
              {piUpgradeOutput && (
                <pre className="sg-log">{piUpgradeOutput}</pre>
              )}
            </section>
            ...
            <section className="sg-section">
              <div className="sg-section-head">
                ...
                <button type="button" className="sg-button" onClick={onOpenWorkspaceMenu}>打开工作区菜单</button>
              </div>
              <div className="sg-kv">
                <div className="sg-kv-row">
                  <span>已记录工作区</span>
            ...
            <section className="sg-section">
              <div className="sg-section-head">
                <span className="sg-section-icon"><Icon name="cpu" /></span>
                <div>
            ...
            <section className="sg-section">
              <div className="sg-section-head">
                <span className="sg-section-icon"><Icon name="info" /></span>
                <div>
